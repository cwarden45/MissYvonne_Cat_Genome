#setwd("E:\\Bastu_Genome\\MissYvonne_Cat_Genome\\Kraken2")
#code is a modified version of https://github.com/cwarden45/Bastu_Cat_Genome/blob/master/Basepaws_Notes/Reformat_Basepaws_WGS2_and_Combine/create_Kracken2_Bracken_table-INTERSPECIES_HOST.R

input_folder = "Kraken-OUT"
Cat   = c("MissYvonne", "Bastu", "Bastu")
SampleSite= c("Oral","Oral","Oral")
output.classification_rate = "Oral3_Kraken2-Bacterial_Classifications.txt"
output.percent_quantified = "Oral3_FILTERED_Braken_genera-heatmap_quantified.txt"
output.counts = "Oral3_FILTERED_Braken_genera-counts.txt"
heatmap.output_quantified = "Oral3_FILTERED_Braken_genera-heatmap_quantified.pdf"
heatmap.output_quantified20 = "Oral3_FILTERED_Braken_genera-heatmap_quantified-TOP20.pdf"
min.percent = 0.5
...
speciesCol=rep("black",length(Cat))
speciesCol[Cat == "Bastu"]="tan"
speciesCol[Cat == "MissYvonne"]="magenta"

siteCol=rep("black",length(Cat))
siteCol[SampleSite == "Oral"]=rainbow(4)[3]
siteCol[SampleSite == "Fecal"]=rainbow(4)[4]

source("../heatmap.3.R")

column_annotation = as.matrix(data.frame(Cat = speciesCol,SampleSite = siteCol))
colnames(column_annotation)=c("Cat","SampleSite")
...
heatmap.3(genus_percent.quantified,   distfun = cor.dist, hclustfun = hclust,
			...
			ColSideColors=column_annotation, ColSideColorsSize=2,
			cexRow=0.5, cexCol=1.5,
			trace="none", margins = c(14,10), dendrogram="both")
legend("topright", pch=15, ncol=1, pt.cex=0.75, cex=0.75,
		legend=c("Fecal","Oral","","Bastu","MissYvonne"),
		col=c(rainbow(4)[4],rainbow(4)[3],"white","tan","magenta"))
dev.off()
