#setwd("E:\\Bastu_Genome\\MissYvonne_Cat_Genome\\ADMIXTURE+RFMix_ReAnalysis_with_Gandolfi-SNP-chip")

cat_of_interest = "MissYvonne"

vcf = "Gandolfi_felCat8_plus_MissYvonne.vcf"
ped = "Gandolfi_felCat8_plus_MissYvonne.ped"
ancestry = "../../Cat_SNP_Chip/FID_breed_codes_fromHasan.txt"
...
points(pc.values$PC1[ancestry.table$ancestry1 == cat_of_interest], pc.values$PC2[ancestry.table$ancestry1 == cat_of_interest], pch=23, cex=2, col="black",bg="red",
	xlab="PC1",ylab="PC2")
legend("right", legend=c("MissYvonne","","Eastern","Western"),col=c("red","white","blue","orange"),
		pch=16, xpd=T, inset=-0.40)		
dev.off()
...
points(pc.values$PC1[ancestry.table$ancestry1 == cat_of_interest], pc.values$PC2[ancestry.table$ancestry1 == cat_of_interest], pch=23, cex=2, col="black",bg="red",
	xlab="PC1",ylab="PC2")
legend("right", legend=c("MissYvonne","","Eastern","Western","Persian"),col=c("red","white","blue","orange","green"),
		pch=16, xpd=T, inset=-0.40)		
dev.off()

#3 ancestry groups + Maine Coon + Siberian
plotCol=rep(adjustcolor( "gray", alpha.f = 0.05),nrow(ancestry.table))
plotCol[ancestry.table$ancestry2 == "Eastern"]=adjustcolor( "blue", alpha.f = 0.15)
plotCol[ancestry.table$ancestry2 == "Western"]=adjustcolor( "orange", alpha.f = 0.30)
plotCol[ancestry.table$ancestry2 == "Persian"]=adjustcolor( "green", alpha.f = 0.15)
plotCol[ancestry.table$breed == "Maine Coon"]="cyan"
plotCol[ancestry.table$breed == "Siberian"]="pink"

png("PCA_ancestry_3groups-plus-Maine-Coon_and_Siberian.png")
par(mar=c(5,5,3,10))
plot(pc.values$PC1, pc.values$PC2, pch=16, col=plotCol,
	xlab="PC1",ylab="PC2")
points(pc.values$PC1[ancestry.table$ancestry1 == cat_of_interest], pc.values$PC2[ancestry.table$ancestry1 == cat_of_interest], pch=23, cex=2, col="black",bg="red",
	xlab="PC1",ylab="PC2")
legend("right", legend=c("MissYvonne","","Eastern","Western","Persian","Maine Coon","Siberian"),col=c("red","white",
																		adjustcolor( "blue", alpha.f = 0.50),
																		adjustcolor( "orange", alpha.f = 0.75),
																		adjustcolor( "green", alpha.f = 0.60),
																		"cyan","pink"),
		pch=16, xpd=T, inset=-0.40)		
dev.off()
...
points(pc.values$PC1[ancestry.table$ancestry1 == cat_of_interest], pc.values$PC2[ancestry.table$ancestry1 == cat_of_interest], pch=23, cex=2, col="black",bg="red",
	xlab="PC1",ylab="PC2")
legend("right", legend=c("MissYvonne","",plot.ancestry),col=c("red","white",breed.colors),
